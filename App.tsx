...
const AlertLog: React.FC<{ alerts: AlertMessage[]; isLoading: boolean }> = ({
  alerts,
  isLoading,
}) => {
    ...
    <div className="flex flex-col h-full bg-gray-800 rounded-lg shadow-inner">
      ...
      <div className="flex-grow p-4 overflow-y-auto space-y-4">
        {alerts.map((alert) => (
          <div
            key={alert.id}
            className={`p-3 rounded-md ${getAlertStyle(alert.type)}`}
          >
            <div className="flex justify-between items-center text-sm mb-1">
              <span
                className={`font-semibold ${
                  alert.type === SignalStatus.Error
                    ? "text-red-400"
                    : alert.type === SignalStatus.Warning
                    ? "text-yellow-400"
                    : "text-green-400"
                }`}
              >
...
const App: React.FC = () => {
  ...
  const possibleIssues = [
    {
      type: SignalStatus.Warning,
      issue: "Slight audio desynchronization detected on channel 3.",
    },
    {
      type: SignalStatus.Error,
      issue: "Critical signal loss on primary satellite uplink.",
    },
    {
      type: SignalStatus.Warning,
      issue:
        "Minor video artifacting (pixelation) observed during high-motion scene.",
    },
    {
      type: SignalStatus.Error,
      issue: "Complete audio dropout on main program feed.",
    },
    {
      type: SignalStatus.Warning,
      issue: "Loudness levels exceed EBU R128 standard by 2 LU.",
    },
    {
      type: SignalStatus.Good,
      issue: "System check complete. All parameters are nominal.",
    },
  ];
  ...
  useEffect(() => {
        ...
        .catch((err) => {
          ...
          setAlerts((prev) => [
            ...prev,
            {
              ...
              type: SignalStatus.Error,
              message:
                "Webcam access denied or unavailable. Displaying placeholder background.",
            },
          ]);
        });
    }
    ...
    setTimeout(() => {
      generateAlert(
        "Monitoring initialized. Primary video and audio feeds are stable.",
        SignalStatus.Good
      );
    }, 1500);
    ...
    <div className="min-h-screen bg-gray-900 text-gray-200 font-sans p-4 lg:p-8">
      <header className="mb-6">
        <h1 className="text-3xl font-bold text-cyan-400">
          AI Broadcast Monitor
        </h1>
        <p className="text-gray-400">
          Real-time integrity analysis of broadcast streams
        </p>
      </header>
      <main
